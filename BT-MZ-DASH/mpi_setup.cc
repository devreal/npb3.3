...
ArrayT1<size_type, int> qcomm_size, qstart2_west , qstart2_east,
...
void mpi_setup()
{
  ...
  proc_gzone_id.allocate(max_zones);
  ...
  if (myid == root) {
   ...
   10       format(' Warning: Requested ',i4,' threads per process,',
    ...
    num_threads = mp;
      ...
      std::cout << "npb_verbose " << npb_verbose << std::endl;
...
//230          continue
  ...
  // exchange global-to-local zone mapping
  MPI_Allreduce(MPI_IN_PLACE, proc_gzone_id.begin(), num_zones, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  ...
  ArrayT1<size_type, size_type>& nz)
{
  ...
  /* Allocate zone halo exchange array using the dynamic size information calculated above */

  size_type max_x_face_size = 0;
  size_type max_y_face_size = 0;

  for (int iz = 1; iz <= x_zones*y_zones; ++iz) {
      max_x_face_size = std::max(max_x_face_size, (ny(iz)-2)*(nz(iz)-2)*5);
      max_y_face_size = std::max(max_y_face_size, (nx(iz)-2)*(nz(iz)-2)*5);
  }

  size_type max_face_size = std::max(max_x_face_size, max_y_face_size);

  int num_zones = proc_num_zones;
  MPI_Allreduce(MPI_IN_PLACE, &num_zones, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);

  halo_array.allocate(dash::size(), num_zones, 4, max_face_size,
                      dash::BLOCKED, dash::NONE, dash::NONE, dash::NONE);

#if 0
  std::cout << "Allocating NArray {" << dash::size() << ", " << num_zones
            << ", " << 4 << ", " << max_face_size << "}, total size: "
            << halo_array.size() << std::endl;
#endif
