...
#define FILENAME_CC   "npbparams_cc.h"
#define DESC_LINE     "c NPROCS = %d CLASS = %c\n"
#define DESC_LINE_CC  "// NPROCS = %d CLASS = %c\n"
#define FINDENT       "        "
#define CONTINUE      "     > "
...
void write_bt_info(FILE *fp, int lang, int nprocs, char class);
void write_lu_info(FILE *fp, int nprocs, char class);
void write_compiler_info(int type, FILE *fp, int lang);
...
void put_string(FILE *fp, int lang, char *name, char *val);
...
{
  FILE *fp_ftn, *fp_cxx;
  fp_ftn = fopen(FILENAME, "w");
  fp_cxx = fopen(FILENAME_CC, "w");
  if (fp_ftn == NULL || fp_cxx == NULL) {
  ...
  switch(type) {
      ...
      case LU:
          ...
          fprintf(fp_ftn, DESC_LINE, nprocs, class);
          fprintf(fp_cxx, DESC_LINE_CC, nprocs, class);
...
c  \n");
          ...
          fprintf(fp_cxx, "\
#ifndef HAVE_NPBPARAMS_CC_H\n \
#define HAVE_NPBPARAMS_CC_H\n \
//  \n\
//  \n\
...
//  \n");
  ...
  switch(type) {
  case SP:
    write_sp_info(fp_ftn, nprocs, class);
    break;
  case BT:
    write_bt_info(fp_ftn, LANG_FTN, nprocs, class);
    write_bt_info(fp_cxx, LANG_CXX, nprocs, class);
    break;
  case LU:
    write_lu_info(fp_ftn, nprocs, class);
    break;
  default:
    printf("setparams: (Internal error): Unknown benchmark type %d\n", type);
    exit(1);
  }
  // TODO convertdouble info is not used in the C++ port
  write_convertdouble_info(type, fp_ftn);
  write_compiler_info(type, fp_ftn, LANG_FTN);
  write_compiler_info(type, fp_cxx, LANG_CXX);
  fprintf(fp_cxx, "#endif // HAVE_NPBPARAMS_CC_H");
  fclose(fp_ftn);
  fclose(fp_cxx);
  return;
}
...
void write_bt_info(FILE *fp, int lang, int nprocs, char class)
{
  ...
  long   max_xysize, max_xybcsize;
  int    max_numzones, max_lsize, nprocs2;
  char   *dt, *ratio, *int_type, *int_type_cxx;
  double ratio_val, dt_val;

  int_type="integer";
  int_type_cxx = "int32_t";
  if      (class == 'S')
  {gx_size = 24; gy_size=24; gz_size=6;
  ...
  {gx_size = 4224; gy_size=3456; gz_size=92;
   ...
   dt = "0.000004d0"; niter = 250; int_type="integer*8"; int_type_cxx="int64_t";}
  else if (class == 'F')
  {gx_size = 12032; gy_size=8960; gz_size=250;
  ...
  if (lang == LANG_FTN) {
    ...
    fprintf(fp, "%sconstexpr const char CLASS = '%c';\n", FINDENT, class);
    fprintf(fp, "%sconstexpr const int num_procs = %d, num_procs2 = %d;\n", FINDENT, nprocs, nprocs2);
    fprintf(fp, "%sconstexpr const int x_zones = %d, y_zones = %d;\n", FINDENT, x_zones, y_zones);
    fprintf(fp, "%sconstexpr const int gx_size = %d, gy_size = %d, gz_size = %d, niter_default = %d;\n",
            FINDENT, gx_size, gy_size, gz_size, niter);
    fprintf(fp, "%sconstexpr const int problem_size = %d;\n", FINDENT, max(max_lsize,gz_size));
    fprintf(fp, "%susing size_type = %s;\n", FINDENT, int_type_cxx);
    fprintf(fp, "%sconstexpr const size_type max_xysize = %ld, max_xybcsize = %ld;\n", FINDENT, max_xysize, max_xybcsize);
    fprintf(fp, "%sconstexpr const size_type proc_max_size = max_xysize*gz_size;\n", FINDENT);
    fprintf(fp, "%sconstexpr const size_type proc_max_size5 = proc_max_size*5;\n", FINDENT);
    fprintf(fp, "%sconstexpr const size_type proc_max_bcsize = max_xybcsize*(gz_size-2);\n", FINDENT);

    fprintf(fp, "%sconstexpr const int max_numzones = %d;\n", FINDENT, max_numzones);
    fprintf(fp, "%sconstexpr const double dt_default = %f, ratio = %f;\n", FINDENT, dt_val, ratio_val);
  }
}
...
setparams: File %s doesn't exist. To build the NAS benchmarks\n\
  ...
  while (fgets(line, LL, deffile) != NULL) {
    ...
    if (lang == LANG_FTN) {
      ...
      check_line(line, "CXX", f77);
      check_line(line, "CXXLINK", flink);
      check_line(line, "CXX_LIB", f_lib);
      check_line(line, "CXX_INC", f_inc);
      check_line(line, "CXXFLAGS", fflags);
      check_line(line, "CXXLINKFLAGS", flinkflags);
    }
  ...
  switch(type) {
      ...
      case BT:
      case LU:
          put_string(fp, lang, "compiletime", compiletime);
          put_string(fp, lang, "npbversion", VERSION);
          put_string(fp, lang, "cs1", f77);
          put_string(fp, lang, "cs2", flink);
          put_string(fp, lang, "cs3", f_lib);
          put_string(fp, lang, "cs4", f_inc);
          put_string(fp, lang, "cs5", fflags);
          put_string(fp, lang, "cs6", flinkflags);
          put_string(fp, lang, "cs7", randfile);
          break;
      default:
          printf("setparams: (Internal error): Unknown benchmark type %d\n",
                                                                         type);
...
void put_string(FILE *fp, int lang, char *name, char *val)
{
  ...
  if (lang == LANG_FTN) {
    ...
    fprintf(fp, "%sconstexpr const char* %s = \"%s\";\n", FINDENT, name, val);
  }
}
