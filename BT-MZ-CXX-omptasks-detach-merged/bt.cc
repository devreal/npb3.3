...
c---------------------------------------------------------------------
*/
    std::cout << "Doing first timestep" << std::endl; 
//#pragma omp parallel
//{
//#pragma omp master
//{
    parallel_master_with_progress([&](){
...
//{
    ...
    //doloop(1, niter, [&](auto step) {
      for (int step = 1; step <= niter; ++step) {
        ...
        #pragma omp task depend(out:u_ptr[0]) default(shared) firstprivate(iz, u_ptr, proc_num_zones)
        {
          int zone = proc_zone_id(iz);
