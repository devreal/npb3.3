...
//#pragma omp for
    ...
    // taskloop uses the C-style iteration space ([a, b))
    // create one task per outer iteration (z is the smallest dimension)
    // TODO: check if necessary to bundle up outer iterations
    dash::tasks::taskloop(1, nz-2+1, dash::tasks::chunk_size(1),
      [=](auto k, auto){
        ...
        // convince the compiler that the pointers are restrict, currently (01/2020)
        // most compilers lose this information in the lambda
        ...
        // multi-dimensional views on the arrays, moved into the task for scoping reasons
