...
*/
    ...
    allocate_halo(nx, ny, nz);

    set_constants();

    // do iz = 1, proc_num_zones
    //for (int iz = 1; iz <= proc_num_zones; ++iz) {
    dash::tasks::taskloop(1, proc_num_zones+1, dash::tasks::chunk_size(1),
      [&](auto iz, auto){
        ...
        //print_zone(&forcing(start5(iz)), nx(zone), nxmax(zone), ny(zone), nz(zone), zone+1, -2);
        //print_zone(&forcing(start5(iz)), nx(zone), nxmax(zone), ny(zone), nz(zone), zone+1, -2);
...
*/
    ...
    dash::tasks::taskloop(1, proc_num_zones+1, dash::tasks::chunk_size(1),
      ...
      [&](auto iz, auto, auto deps){
        deps = dash::tasks::out(u(start5(iz)));
      }); // end do
