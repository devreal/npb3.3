         ...
         SizeT nx, SizeT nxmax, SizeT ny, SizeT nz)
{
  ...
  // wait for above tasks to complete as z_solve substantially different
  // iteration characteristics than the above, so we cannot interleave these
  dash::tasks::complete();

  z_solve(rho_i, qs, square, u, rhs, nx, nxmax, ny, nz);

  // wait for z_solve tasks to complete as z_solve has substantially different
  // iteration characteristics than add()
  dash::tasks::complete();
