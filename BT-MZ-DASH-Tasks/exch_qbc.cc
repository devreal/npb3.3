...
*/
  ...
  // wait for all processes to complete their previous reads
  ...
  for (int iz = 1; iz <= proc_num_zones; ++iz) {
      ...
      {
        auto *target = &halo_array.local(0, iz-1, ZONE_WEST, 0);
        dash::tasks::async("COPYOUT_WEST", [&, iz, target, nnx, nnxmax, nny, nnz](){
            copy_x_face(&u(start5(iz)), target,
                        nnx, nnxmax, nny, nnz, 1, DIR_OUT);
          },
          dash::tasks::in(u(start5(iz))),
          dash::tasks::out(target[0]));
      }

      {
        ...
        auto *target = &halo_array.local(0, iz-1, ZONE_EAST, 0);
        dash::tasks::async("COPYOUT_EAST", [&, iz, target, nnx, nnxmax, nny, nnz](){
            copy_x_face(&u(start5(iz)),target,
                        nnx, nnxmax, nny, nnz, nnx-2, DIR_OUT);
          },
          dash::tasks::in(u(start5(iz))),
...

      {
        ...
        auto *target = &halo_array.local(0, iz-1, ZONE_SOUTH, 0);
        dash::tasks::async("COPYOUT_SOUTH", [&, iz, target, nnx, nnxmax, nny, nnz](){
          copy_y_face(&u(start5(iz)), target,
                      nnx, nnxmax, nny, nnz, 1, DIR_OUT);
        },
        dash::tasks::in(u(start5(iz))),
        dash::tasks::out(target[0]));
...

      {
        ...
        auto *target = &halo_array.local(0, iz-1, ZONE_NORTH, 0);
        dash::tasks::async("COPYOUT_NORTH", [&, iz, target, nnx, nnxmax, nny, nnz](){
          copy_y_face(&u(start5(iz)), target,
                      nnx, nnxmax, nny, nnz, nny-2, DIR_OUT);
        },
        dash::tasks::in(u(start5(iz))),
  ...
  for (int iz = 1; iz <= proc_num_zones; ++iz) {
      ...
      size_type x_face_size = (ny(iz)-2)*(nz(iz)-2)*5;
      size_type y_face_size = (nx(iz)-2)*(nz(iz)-2)*5;
  ...
  auto get_gptr = [&](int gzone, int ip, int zone_dir){
    auto lzone = static_cast<long>(proc_gzone_id(gzone))-1;
    auto offset = pattern.global_at({ip, lzone, zone_dir, 0});
    auto gptr = halo_array.begin() + offset;
    return gptr;
  };
  ...
  for (int iz = 1; iz <= proc_num_zones; ++iz) {
      ...
      */
      ...
      {
        int izone_west = iz_west(zone_no);
        auto gptr = get_gptr(izone_west, ip_west, ZONE_EAST);
        auto local_src = &halo_array.local(0, proc_gzone_id(izone_west)-1, ZONE_EAST, 0);
        // TODO: use automatic buffer handling
        dash::tasks::async("COPYIN_WEST", [&, iz, zone_no, nnx, nnxmax, nny, nnz, ip_west, local_src](){
            auto src = ip_west != myid ? &qbc_in(qstart2_west(zone_no)) : local_src;
            copy_x_face(&u(start5(iz)), src,
                        nnx, nnxmax, nny, nnz, 0, DIR_IN);
          },
          dash::tasks::copyin_r(gptr, x_face_size, ip_west != myid ? &qbc_in(qstart2_west(zone_no)) : nullptr),
          //dash::tasks::in(gptr),
          dash::tasks::in(local_src[0]),
          // use input dependency as sentinel to block next adi step until all communication is done
          dash::tasks::in(u(start5(iz)))
        );
      }
      ...
      */
      ...
      {
        int izone_east = iz_east(zone_no);
        auto gptr = get_gptr(izone_east, ip_east, ZONE_WEST);
        auto local_src = &halo_array.local(0, proc_gzone_id(izone_east)-1, ZONE_WEST, 0);
        // TODO: use automatic buffer handling
        dash::tasks::async("COPYIN_EAST", [&, iz, zone_no, nnx, nnxmax, nny, nnz, ip_east, local_src](){
            auto src = ip_east != myid ? &qbc_in(qstart2_east(zone_no)) : local_src;
            copy_x_face(&u(start5(iz)), src,
                        nnx, nnxmax, nny, nnz, nnx-1, DIR_IN);
          },
          dash::tasks::copyin_r(gptr, x_face_size, ip_east != myid ? &qbc_in(qstart2_east(zone_no)) : nullptr),
          //dash::tasks::in(gptr),
          dash::tasks::in(local_src[0]),
          // use input dependency as sentinel to block next adi step until all communication is done
          dash::tasks::in(u(start5(iz)))
      ...
      */
      ...
      {
        int jzone_south = iz_south(zone_no);
        auto gptr = get_gptr(jzone_south, ip_south, ZONE_NORTH);
        auto local_src = &halo_array.local(0, proc_gzone_id(jzone_south)-1, ZONE_NORTH, 0);
        // TODO: use automatic buffer handling
        dash::tasks::async("COPYIN_SOUTH", [&, iz, zone_no, nnx, nnxmax, nny, nnz, ip_south, local_src](){
            auto src = ip_south != myid ? &qbc_in(qstart2_south(zone_no)) : local_src;
            copy_y_face(&u(start5(iz)), src,
                        nnx, nnxmax, nny, nnz, 0, DIR_IN);

          },
          dash::tasks::copyin_r(gptr, y_face_size, ip_south != myid ? &qbc_in(qstart2_south(zone_no)) : nullptr),
          //dash::tasks::in(gptr),
          dash::tasks::in(local_src[0]),
          // use input dependency as sentinel to block next adi step until all communication is done
          dash::tasks::in(u(start5(iz)))
      ...
      */
      ...
      {
        int jzone_north = iz_north(zone_no);
        auto gptr = get_gptr(jzone_north, ip_north, ZONE_SOUTH);
        auto local_src = &halo_array.local(0, proc_gzone_id(jzone_north)-1, ZONE_SOUTH, 0);
        // TODO: use automatic buffer handling
        dash::tasks::async("COPYIN_NORTH", [&, iz, zone_no, nnx, nnxmax, nny, nnz, ip_north, local_src](){
          auto src = ip_north != myid ? &qbc_in(qstart2_north(zone_no)) : local_src;
          copy_y_face(&u(start5(iz)), src,
                      nnx, nnxmax, nny, nnz, nny-1, DIR_IN);

          },
          dash::tasks::copyin_r(gptr, y_face_size, ip_north != myid ? &qbc_in(qstart2_north(zone_no)) : nullptr),
          //dash::tasks::in(gptr),
          dash::tasks::in(local_src[0]),
          // use input dependency as sentinel to block next adi step until all communication is done
          dash::tasks::in(u(start5(iz)))
