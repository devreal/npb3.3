 ...
 *
 * TODO: Both Intel (19.1) and GNU (8.3) compilers screw up this abstraction.
 *       Come back in 2030 to try again...
 */
...
void print_zone(
  ValueT* u_ptr,
  SizeT nx, SizeT nxmax, SizeT ny, SizeT nz, int step)
{

  ArrayViewT4<ValueT, SizeT> u(u_ptr, 5, nxmax, ny, nz);

  std::cout << "------ Printing U at step " << step << " -------" << std::fixed << std::endl;
